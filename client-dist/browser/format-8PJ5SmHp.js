...
function p(t) {
  return (e, n = {}) => {
    ...
    if (!o)
      return null;
    const i = o[0], u = r && t.parsePatterns[r] || t.parsePatterns[t.defaultParseWidth], f = Array.isArray(u) ? ot(u, (c) => c.test(i)) : (
...
function C(t, e) {
  ...
  const u = P(i, e), f = g(e?.in || t, 0);
  f.setFullYear(r, 0, o), f.setHours(0, 0, 0, 0);
  const m = P(f, e);
  return +n >= +u ? r + 1 : +n >= +m ? r : r - 1;
}
function kt(t, e) {
  const n = O(), r = e?.firstWeekContainsDate ?? e?.locale?.options?.firstWeekContainsDate ?? n.firstWeekContainsDate ?? n.locale?.options?.firstWeekContainsDate ?? 1, a = C(t, e), o = g(e?.in || t, 0);
  return o.setFullYear(a, 0, r), o.setHours(0, 0, 0, 0), P(o, e);
}
function xt(t, e) {
  const n = h(t, e?.in), r = +P(n, e) - +kt(n, e);
  return Math.round(r / 6048e5) + 1;
}
function s(t, e) {
...
function Lt(t, e, n) {
  const r = O(), a = r.locale ?? Pt, o = r.firstWeekContainsDate ?? r.locale?.options?.firstWeekContainsDate ?? 1, i = r.weekStartsOn ?? r.locale?.options?.weekStartsOn ?? 0, u = h(t, n?.in);
  if (!L(u))
