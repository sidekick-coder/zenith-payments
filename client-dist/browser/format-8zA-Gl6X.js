...
function v(e) {
	return !(!_(e) && typeof e != "number" || isNaN(+o(e)));
}
//#endregion
...
function D(e) {
	return (t, n = {}) => {
		...
		if (!a) return null;
		let o = a[0], s = r && e.parsePatterns[r] || e.parsePatterns[e.defaultParseWidth], c = Array.isArray(s) ? k(s, (e) => e.test(o)) : O(s, (e) => e.test(o)), l;
...
function F(e, t) {
	let n = c(), r = t?.firstWeekContainsDate ?? t?.locale?.options?.firstWeekContainsDate ?? n.firstWeekContainsDate ?? n.locale?.options?.firstWeekContainsDate ?? 1, i = P(e, t), o = a(t?.in || e, 0);
	return o.setFullYear(i, 0, r), o.setHours(0, 0, 0, 0), l(o, t);
}
//#endregion
//#region node_modules/date-fns/getWeek.js
function I(e, n) {
	let r = o(e, n?.in), i = l(r, n) - +F(r, n);
	return Math.round(i / t) + 1;
}
//#endregion
...
var te = /[yYQqMLwIdDecihHKkms]o|(\w)\1*|''|'(''|[^'])+('|$)|./g, ne = /P+p+|P+|p+|''|'(''|[^'])+('|$)|./g, re = /^'([^]*?)'?$/, ie = /''/g, ae = /[a-zA-Z]/;
function oe(e, t, n) {
	let r = c(), i = n?.locale ?? r.locale ?? j, a = n?.firstWeekContainsDate ?? n?.locale?.options?.firstWeekContainsDate ?? r.firstWeekContainsDate ?? r.locale?.options?.firstWeekContainsDate ?? 1, s = n?.weekStartsOn ?? n?.locale?.options?.weekStartsOn ?? r.weekStartsOn ?? r.locale?.options?.weekStartsOn ?? 0, l = o(e, n?.in);
	if (!v(l)) throw RangeError("Invalid time value");
	let u = t.match(ne).map((e) => {
		let t = e[0];
		if (t === "p" || t === "P") {
			let n = K[t];
	...
	return u.map((r) => {
		...
		let a = r.value;
		(!n?.useAdditionalWeekYearTokens && Z(a) || !n?.useAdditionalDayOfYearTokens && X(a)) && Q(a, t, String(e));
		let o = B[a[0]];
