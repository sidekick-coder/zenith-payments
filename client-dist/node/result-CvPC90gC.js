...
var result_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const config = computed(() => {
			...
			if (resultType === "cancelled") return {
				icon: "Ban",
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(unref(Card), { class: "w-full max-w-md" }, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(ssrRenderComponent(unref(CardHeader), { class: "text-center" }, {
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) {
									_push(`<div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-secondary"${_scopeId}>`);
									_push(ssrRenderComponent(Icon, {
										name: config.value.icon,
										class: ["h-8 w-8", config.value.iconClass]
									}, null, _parent, _scopeId));
									_push(`</div>`);
									_push(ssrRenderComponent(unref(CardTitle), { class: "text-2xl" }, {
										default: withCtx((_, _push, _parent, _scopeId) => {
											if (_push) _push(`${ssrInterpolate(config.value.title)}`);
											else return [createTextVNode(toDisplayString(config.value.title), 1)];
										}),
										_: 1
									}, _parent, _scopeId));
									_push(ssrRenderComponent(unref(CardDescription), { class: "text-base" }, {
										default: withCtx((_, _push, _parent, _scopeId) => {
											if (_push) _push(`${ssrInterpolate(message.value || config.value.description)}`);
											else return [createTextVNode(toDisplayString(message.value || config.value.description), 1)];
										}),
										_: 1
									}, _parent, _scopeId));
								} else return [
									createVNode("div", { class: "mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-secondary" }, [createVNode(Icon, {
										name: config.value.icon,
										class: ["h-8 w-8", config.value.iconClass]
									}, null, 8, ["name", "class"])]),
									createVNode(unref(CardTitle), { class: "text-2xl" }, {
										default: withCtx(() => [createTextVNode(toDisplayString(config.value.title), 1)]),
										_: 1
									}),
									createVNode(unref(CardDescription), { class: "text-base" }, {
										default: withCtx(() => [createTextVNode(toDisplayString(message.value || config.value.description), 1)]),
										_: 1
									})
								];
							}),
						...
						_push(ssrRenderComponent(unref(CardContent), { class: "flex justify-center" }, {
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) _push(ssrRenderComponent(Button, {
									...
									default: withCtx((_, _push, _parent, _scopeId) => {
										if (_push) {
											...
											_push(` ${ssrInterpolate(_ctx.$t("Go to Home"))}`);
					...
					} else return [createVNode(unref(CardHeader), { class: "text-center" }, {
						default: withCtx(() => [
							createVNode("div", { class: "mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-secondary" }, [createVNode(Icon, {
								name: config.value.icon,
								class: ["h-8 w-8", config.value.iconClass]
...
//#endregion
//#region modules/mod/client/pages/result.vue
var _sfc_setup = result_vue_vue_type_script_setup_true_lang_default.setup;
result_vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
