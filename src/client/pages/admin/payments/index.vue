...
<template>
    <PageCrud v-model:fetch-query="fetchQuery" :title="$t('Payments')" :description="$t('View payment records here.')"
        :columns="columns" :serialize="row => Payment.from(row)" fetch="/api/zpayments/payments" :actions="[]">
        <template #header-append>
            <Card class="mb-4">
                <CardContent class="pt-6">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <Select v-model="fetchQuery.status" :label="$t('Status')" :placeholder="$t('All')"
                            :options="Payment.STATUS" label-key="label" value-key="id" clearable />

                        <div class="flex flex-col gap-2">
                            <label class="text-sm font-medium">
                                {{ $t('Start Date') }}
                            </label>
                            <DatePicker v-model="fetchQuery.start_date" mode="datetime" clearable />
                        </div>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <DatePicker v-model="fetchQuery.end_date" mode="datetime" clearable />
                        </div>
        ...
        <template #row-status="{ row }">
            <Badge :style="{ '--color': row.statusColor }" class="bg-[var(--color)] text-white">
                {{ row.statusLabel }}
            </Badge>
        </template>

        <template #prepend-actions="{ row }">
            <Button size="sm" tag="a" :href="`/api/zpayments/payments/${row.id}/process`" target="_blank">
                {{ $t('Process') }}
            </Button>
        </template>
    </PageCrud>
